...
static conf_option_t conf_server_option[] = 
{
    ...
    { string_make("bind_for_dn"), conf_parse_bind,
        OPE_EQUAL, offsetof(conf_server_t, bind_for_dn) },
    ...
    { string_make("ot_paxos"), conf_parse_string,
        OPE_EQUAL, offsetof(conf_server_t, ot_paxos) },

    { string_make("editlog_dir"), conf_parse_string,
        OPE_EQUAL, offsetof(conf_server_t, editlog_dir) },

    { string_make("fsimage_dir"), conf_parse_string,
        OPE_EQUAL, offsetof(conf_server_t, fsimage_dir) },

    { string_make("paxos_group_num"), conf_parse_int,
        OPE_EQUAL, offsetof(conf_server_t, paxos_group_num) },

    { string_make("checkpoint_num"), conf_parse_int,
        OPE_EQUAL, offsetof(conf_server_t, checkpoint_num) },

    { string_make("index_num"), conf_parse_int,
        OPE_EQUAL, offsetof(conf_server_t, index_num) },

	{ string_make("dn_timeout"), conf_parse_int,
        OPE_EQUAL, offsetof(conf_server_t, dn_timeout) },
...
static void *conf_server_init(pool_t *pool)
{
    ...
    if (array_init(&sconf->bind_for_dn, pool, CONF_SERVER_BIND_N, 
        sizeof(server_bind_t)) != DFS_OK) 
    {
        return NULL;
    }
